// CRUD genérico das entidades (mesmo contrato HTTP do Base44) + regras de acesso (RLS).
...
// Converte a regra do Base44 para um filtro comum (o mesmo formato das consultas)
...
export async function listRecords(entity: string, auth: AuthContext, opts: ListOptions): Promise<Rec[]> {
  ...
  return rows.map((r) => toRecord(r, opts.fields));
}
