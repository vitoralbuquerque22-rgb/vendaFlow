...
import { SOCKET_EVENTS } from '@/contexts/telefonia/TELEFONIA_ENGINE';
...
export function useMonitoramentoRealTime({ empresaId, habilitado = true }) {
  ...
  // Avisos para o gestor agir. O 3C repete list-empty a cada tentativa de discagem:
  // mostramos no máximo um aviso por lista (ou por tipo) a cada 10 minutos.
  const avisosMostradosRef = useRef(new Map());
  const onAvisoOperacional = useCallback((evento, data) => {
    const chave = `${evento}:${data?.list?.id ?? ''}`;
    const agora = Date.now();
    if (agora - (avisosMostradosRef.current.get(chave) ?? 0) < 10 * 60 * 1000) return;
    avisosMostradosRef.current.set(chave, agora);

    if (evento === SOCKET_EVENTS.LIST_EMPTY) {
      const nome = data?.list?.name ? ` "${data.list.name}"` : '';
      toast.warning(`A lista de contatos${nome} acabou. Suba uma nova lista para a campanha continuar discando.`, { duration: 20000 });
    } else if (evento === SOCKET_EVENTS.REACHED_MAX_ONLINE_AGENTS) {
      toast.error('Limite de agentes logados no 3C Plus atingido. Novos agentes não conseguem entrar na campanha.', { duration: 20000 });
    }
  }, []);
