...
export default function LeadsExternos({ onImportarLead }) {
  ...
  const { data: leadsExternos = [], isLoading, refetch, isFetching } = useQuery({
    ...
    queryFn: async () => {
      ...
      const LOTE = 500;
      while (true) {
        const lote = await api.entities.Lead.filter({ empresaId, fonte_externa: true }, "-created_date", LOTE, skip);
  ...
  const webhookUrl = `${window.location.origin}/api/functions/webhookReceberLead?empresaId=${empresaId || "SEU_EMPRESA_ID"}`;
