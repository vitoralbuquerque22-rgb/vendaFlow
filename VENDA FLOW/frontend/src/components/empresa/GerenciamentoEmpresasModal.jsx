...
export default function GerenciamentoEmpresasModal({
  ...
  const createMutation = useMutation({
    mutationFn: async (data) => {
      ...
      const response = await api.functions.invoke('criarEmpresaComConvite', data);
  ...
  const updateMutation = useMutation({
    mutationFn: (data) =>
      api.entities.Empresa.update(empresa.id, data),
    onSuccess: () => {
  ...
  const { data: colaboradores = [], refetch, isLoading: colabLoading } = useQuery({
   ...
   queryFn: async () => {
     ...
     const todos = await api.entities.VinculoEmpresa.list();
