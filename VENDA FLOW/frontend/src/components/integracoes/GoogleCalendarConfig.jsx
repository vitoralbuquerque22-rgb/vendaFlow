...
export default function GoogleCalendarConfig() {
  ...
  useEffect(() => {
    ...
    setTokenPersistence({
      onTokenSaved: async (token) => {
        ...
        await api.entities.Integracao.update(integracao.id, { configuracao: config });
  ...
  const salvarMutation = useMutation({
    mutationFn: async () => {
      ...
      if (integracao?.id) {
        return api.entities.Integracao.update(integracao.id, config);
      }
      return api.entities.Integracao.create(config);
    },
    onSuccess: () => {
