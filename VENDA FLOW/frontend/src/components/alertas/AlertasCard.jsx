...
export default function AlertasCard({ limitarAlertas = 5, mostrarTodos = false }) {
  ...
  const { data: alertas = [] } = useQuery({
    ...
    queryFn: async () => {
      let alertasData = await api.entities.Alerta.filter({ resolvido: false });
  ...
  const marcarVisualizadoMutation = useMutation({
    mutationFn: ({ id, visualizado_por }) =>
      api.entities.Alerta.update(id, { visualizado_por }),
    onSuccess: () => {
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              onClick={async () => {
                ...
                await Promise.all(
                  alertas.map((a) =>
                    api.entities.Alerta.update(a.id, {
                      resolvido: true,
                      resolvido_por: user?.email,
                      resolvido_em: new Date().toISOString(),
                    })
