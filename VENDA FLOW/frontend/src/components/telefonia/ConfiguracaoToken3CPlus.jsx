...
export default function ConfiguracaoToken3CPlus() {
  ...
  useEffect(() => {
    if (!empresaId) return;
    api.entities.Integracao.filter({ empresaId, tipo: 'telefonia', ativa: true })
      .then(integracoes => {
  ...
  const validarToken = async () => {
    ...
    try {
      ...
      const resp = await api.functions.invoke('testar3CPlus', {
        token: token.trim(),
        dominio: dominioApp,
  ...
  const salvarToken = async () => {
    ...
    setSalvando(true);
    try {
      const resp = await api.functions.invoke('salvarTokenAgente3CPlus', {
        token: token.trim(),
        ramal: ramal.trim(),
      });
      if (!resp.data?.success) {
