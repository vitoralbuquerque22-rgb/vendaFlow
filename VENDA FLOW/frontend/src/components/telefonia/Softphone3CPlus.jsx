...
export default function Softphone3CPlus({ telefonia, onCampanhaAtiva }) {
  ...
  const { data: userProfile } = useQuery({
    ...
    staleTime: 30000,
    refetchOnMount: true,
    // Polling só enquanto o perfil não tem ramal/id 3C — para quando encontrar
    refetchInterval: (query) => {
      const p = query.state.data;
      if (p?.id_3cplus || p?.ramal_3cplus || p?.token_3cplus) return false;
      return 30000;
    },
  });
  ...
  useEffect(() => {
    ...
    async function carregarConfig() {
      try {
        ...
        // O backend age pelo agente com o token de serviço + o id 3C do SDR (resolvido pelo ramal).
        // Nenhum token vem para o navegador: basta saber se o SDR está cadastrado como agente.
        setAgenteHabilitado(!!(userProfile?.id_3cplus || userProfile?.ramal_3cplus || userProfile?.token_3cplus || cfg?.mapeamento_ramais?.[(user.email || '').toLowerCase()]));
      } catch (e) {
  ...
  useEffect(() => {
    if (!extensaoInstalada || !agenteHabilitado || !dominioApp || !empresaId) return;
    // O ramal (RamalWebRTC) busca as próprias credenciais SIP no backend pela empresa
    window.postMessage({
      type: 'VENDAFLOW_SET_AGENT',
      config: { empresaId, dominio: dominioApp },
    }, '*');
  }, [extensaoInstalada, agenteHabilitado, dominioApp, empresaId]);
  ...
  useEffect(() => {
    ...
    prevEmLigacaoRef.current = emLigacao;
  }, [emLigacao, tecladoAberto, campanhaAtiva, agenteHabilitado, manualCallSession, manualCallStatus]);
  ...
  const toggleTeclado = async () => {
    ...
    // T4: sem campanha → abre/fecha normalmente sem chamar API
    if (!campanhaAtiva || !agenteHabilitado) {
      setTecladoAberto(novoEstado);
      return;
    }
  ...
  const discarManual = async () => {
    ...
    if (isGestorOuAdmin) {
      ...
      if (!ramal) {
        ...
        toast.error("Ramal 3C Plus não configurado. Acesse Perfil → Dados Pessoais e preencha o campo Ramal 3C Plus.");
        return;
      }
      if (leadIdAtual) {
        setDiscandoManual(true);
        try {
          const resp = await api.functions.invoke("ligarAgoraLead3CPlus", {
            empresaId,
            lead_id: leadIdAtual,
  ...
  const entrarNaCampanha = async () => {
    if (!dominioApp || !agenteHabilitado) return;
    const ramal = userProfile?.ramal_3cplus;
    if (!ramal) {
    ...
    try {
      ...
      try {
        await loginCampanha(campanhaId, { dominio: dominioApp });
  ...
  return (
    <div className={cn(
      ...
      !campanhaAtiva && agenteHabilitado && !emLigacao && "border-amber-500/20",
      !agenteHabilitado     && "border-slate-800/60",
    )}>
      {/* ── Header ──────────────────────────────────────── */}
      <div className="flex items-center justify-between px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Botão pausa */}
          {!emLigacao && !carregandoConfig && agenteHabilitado && !isGestorOuAdmin && campanhaAtiva && (
            <Button
      ...
      {!isGestorOuAdmin && agenteHabilitado && !emLigacao && !carregandoConfig && campanhaAtiva && (
      ...
      {!isGestorOuAdmin && agenteHabilitado && !emLigacao && !carregandoConfig && (
      ...
      {isGestorOuAdmin && !agenteHabilitado && !carregandoConfig && (
      ...
      {/* ── Sem token ────────────────────────────────────── */}
      {!carregandoConfig && !agenteHabilitado && !isGestorOuAdmin && (
