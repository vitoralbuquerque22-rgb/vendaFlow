...
export default function LeadCallAttendanceModal({ telefonia, callSession, onClose }) {
  ...
  const { data: lead } = useQuery({
    ...
    queryFn: async () => {
      ...
      const leads = await api.entities.Lead.filter({ id: callSession.lead_id });
      return leads[0] || null;
    },
  ...
  const { data: atividades = [] } = useQuery({
    ...
    queryFn: () => lead?.id
      ? api.entities.Atividade.filter({ lead_id: lead.id }, "-created_date", 50)
      : [],
    enabled: !!lead?.id,
  ...
  const { data: spinAnteriores = [] } = useQuery({
    ...
    queryFn: () => lead?.id
      ? api.entities.SpinResposta.filter({ lead_id: lead.id }, "-created_date", 5)
      : [],
    enabled: !!lead?.id,
  });
