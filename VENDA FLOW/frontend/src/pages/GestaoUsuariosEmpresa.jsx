...
export default function GestaoUsuariosEmpresa() {
  ...
  const { data: empresa } = useQuery({
    ...
    queryFn: async () => {
      ...
      return await api.entities.Empresa.filter({ id: empresaId });
  ...
  const { data: convites = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!empresa?.[0]?.id) return [];
      const todos = await api.entities.ConviteEmpresa.filter({ empresaId: empresa[0].id });
  ...
  const desativarMutation = useMutation({
    mutationFn: async (vinculoId) => await api.entities.VinculoEmpresa.update(vinculoId, { status: "inativo" }),
  ...
  const alterarPapelMutation = useMutation({
    mutationFn: async ({ vinculoId, novoPapel, userEmail }) => {
      await api.entities.VinculoEmpresa.update(vinculoId, { papel: novoPapel });
      const profiles = await api.entities.UserProfile.filter({ user_email: userEmail, empresaId: empresa[0]?.id });
      if (profiles.length > 0) {
        await api.entities.UserProfile.update(profiles[0].id, { role: novoPapel });
      }
    },
    onSuccess: () => {
  ...
  const reativarMutation = useMutation({
    mutationFn: async (vinculoId) => await api.entities.VinculoEmpresa.update(vinculoId, { status: "ativo" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["usuarios-empresa"] });
  ...
  const deletarConviteMutation = useMutation({
    mutationFn: async (conviteId) => await api.entities.ConviteEmpresa.delete(conviteId),
  ...
  const reenviarConviteMutation = useMutation({
    mutationFn: async (conviteAntigo) => {
      await api.entities.ConviteEmpresa.delete(conviteAntigo.id);
