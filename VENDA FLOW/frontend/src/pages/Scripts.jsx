...
export default function Scripts() {
  ...
  const { data: produtos = [] } = useQuery({
    ...
    queryFn: () => empresaId ? api.entities.Produto.filter({ empresaId }) : [],
    enabled: !!empresaId,
  });

  const criarMutation = useMutation({
    mutationFn: (data) => api.entities.Script.create(data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["scripts"] }); toast.success("Script criado!"); },
  });

  const atualizarMutation = useMutation({
    mutationFn: ({ id, data }) => api.entities.Script.update(id, data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["scripts"] }); toast.success("Script atualizado!"); },
  });

  const deletarMutation = useMutation({
    mutationFn: (id) => api.entities.Script.delete(id),
