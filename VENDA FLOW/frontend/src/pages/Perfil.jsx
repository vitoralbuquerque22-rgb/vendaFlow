...
export default function Perfil() {
  ...
  const { data: atividadesHistoricas = [] } = useQuery({
    ...
    queryFn: () => api.entities.Atividade.filter({ sdr_email: user.email }),
    enabled: !!user?.email,
  });

  const { data: tarefasHistoricas = [] } = useQuery({
    queryKey: ["tarefas-historicas", user?.email],
    queryFn: () => api.entities.Tarefa.filter({ sdr_email: user.email }),
    enabled: !!user?.email,
  });

  const { data: leadsHistoricos = [] } = useQuery({
    queryKey: ["leads-historicos", user?.email],
    queryFn: () => api.entities.Lead.filter({ sdr_responsavel: user.email }),
    enabled: !!user?.email,
  });
  ...
  const { data: todasAtividades = [] } = useQuery({
    ...
    queryFn: async () => {
      const ativs = await api.entities.Atividade.list("-created_date", 10000);
