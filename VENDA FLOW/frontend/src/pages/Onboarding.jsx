...
export default function Onboarding() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setUser(userData);
        const vinculos = await api.entities.VinculoEmpresa.filter({ userEmail: userData.email, status: "ativo" });
  ...
  const finalizar = async () => {
    ...
    try {
      ...
      const empresa = await api.entities.Empresa.create({
      ...
      await api.auth.updateMe({ empresaAtualId: empresa.id, full_name: dadosPerfil.nomeCompleto });
