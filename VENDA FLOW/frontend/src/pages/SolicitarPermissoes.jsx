...
function CommentsSection({ solicitacao, user }) {
  ...
  const addCommentMutation = useMutation({
    mutationFn: async ({ id, text }) => {
      const s = await api.entities.PermissionChangeRequest.get(id);
      ...
      return api.entities.PermissionChangeRequest.update(id, { comments });
    },
    onSuccess: () => {
      setCommentText("");
...
export default function SolicitarPermissoes() {
  ...
  const { data: minhas = [], refetch: refetchMinhas } = useQuery({
    ...
    queryFn: () => api.entities.PermissionChangeRequest.filter({ requester_email: user.email }),
    enabled: !!user?.email,
  ...
  const { data: paraMim = [], refetch: refetchParaMim } = useQuery({
    ...
    queryFn: () => api.entities.PermissionChangeRequest.filter({ target_email: user.email }),
    enabled: !!user?.email,
    refetchInterval: 30000,
    select: data => [...data].sort((a, b) => new Date(b.created_date) - new Date(a.created_date)),
  });
  ...
  const atualizar = (id, patch) =>
    api.entities.PermissionChangeRequest.update(id, patch);
  ...
  const handleEnviar = async () => {
    ...
    setEnviando(true);
    try {
      await api.entities.PermissionChangeRequest.create({
