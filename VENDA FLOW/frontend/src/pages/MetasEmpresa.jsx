...
export default function MetasEmpresa() {
  ...
  const { data: configEmpresa } = useQuery({
    ...
    queryFn: async () => {
      const configs = await api.entities.ConfiguracaoMeta.list();
  ...
  const salvarMetasEmpresaMutation = useMutation({
    mutationFn: async (data) => {
      if (configEmpresa) {
        return api.entities.ConfiguracaoMeta.update(configEmpresa.id, data);
      }
      return api.entities.ConfiguracaoMeta.create(data);
    },
    onSuccess: () => {
