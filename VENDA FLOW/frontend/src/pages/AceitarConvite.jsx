...
export default function AceitarConvite() {
  ...
  useEffect(() => {
    const carregar = async () => {
      ...
      const convites = await api.entities.ConviteEmpresa.filter({ codigo });
      ...
      if (c.expiraEm && new Date(c.expiraEm) < new Date()) {
        await api.entities.ConviteEmpresa.update(c.id, { status: "expirado" });
      ...
      try {
        const empresas = await api.entities.Empresa.filter({ id: c.empresaId });
  ...
  const handleAceitar = async () => {
    ...
    try {
      ...
      const vinculosExistentes = await api.entities.VinculoEmpresa.filter({
        empresaId,
      ...
      if (vinculosExistentes.length > 0) {
        ...
        if (vinculo.status === "ativo") {
          ...
          await api.entities.ConviteEmpresa.update(convite.id, {
            status: "aceito",
          ...
          await api.entities.VinculoEmpresa.update(vinculo.id, {
            status: "ativo",
            papel: convite.papel,
          });
          jaEraMembro = true;
      ...
      try {
        await api.functions.invoke("autoAssignProfile", {
