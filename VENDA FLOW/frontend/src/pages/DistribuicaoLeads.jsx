...
export default function DistribuicaoLeads() {
  ...
  const { data: regras = [] } = useQuery({
    ...
    queryFn: () => api.entities.RegraDistribuicao.filter({ empresaId }, "-prioridade"),
    enabled: !!empresaId,
  });
  ...
  const { data: cadencias = [] } = useQuery({
    ...
    queryFn: () => api.entities.Cadencia.filter({ empresaId, ativa: true }),
    enabled: !!empresaId,
  });
  ...
  const { data: leadsHoje = [] } = useQuery({
    ...
    queryFn: () => api.entities.Lead.filter({ empresaId }, "-created_date", 500),
  ...
  const { data: atividadesHoje = [] } = useQuery({
    ...
    queryFn: async () => {
      const todas = await api.entities.Atividade.filter({ empresaId }, "-created_date", 200);
  ...
  const { data: vinculos = [] } = useQuery({
    ...
    queryFn: () => api.entities.VinculoEmpresa.filter({ empresaId, status: "ativo" }),
    enabled: !!empresaId && isTempoReal,
  });
  ...
  const criarMutation = useMutation({
    mutationFn: (data) => api.entities.RegraDistribuicao.create({ ...data, empresaId }),
  ...
  const atualizarMutation = useMutation({
    mutationFn: ({ id, data }) => api.entities.RegraDistribuicao.update(id, data),
  ...
  const deletarMutation = useMutation({
    mutationFn: (id) => api.entities.RegraDistribuicao.delete(id),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["regras-distribuicao", empresaId] }); toast.success("Regra excluída!"); },
  });
  const toggleMutation = useMutation({
    mutationFn: ({ id, ativa }) => api.entities.RegraDistribuicao.update(id, { ativa }),
