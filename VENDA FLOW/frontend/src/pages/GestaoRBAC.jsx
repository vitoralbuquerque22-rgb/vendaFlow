...
export default function GestaoRBAC() {
  ...
  const { data: vinculos = [] } = useQuery({
    ...
    queryFn: () => api.entities.VinculoEmpresa.filter({ empresaId }),
    enabled: !!empresaId,
  });
  ...
  const { data: solicitacoes = [] } = useQuery({
    ...
    queryFn: () => api.entities.PermissionChangeRequest.filter({ empresaId }, "-created_date"),
    enabled: !!empresaId,
  });

  const { data: logs = [] } = useQuery({
    queryKey: ["rbacLogs", empresaId],
    queryFn: () => api.entities.RBACLog.filter({ empresaId }, "-created_date", 100),
    enabled: !!empresaId,
  });
  ...
  const processarSolicitacaoMutation = useMutation({
    mutationFn: (data) => api.functions.invoke('processPermissionRequest', data),
    onSuccess: () => {
  ...
  const excluirPerfilMutation = useMutation({
    mutationFn: async ({ user_email, empresaId: empId }) => {
      await api.functions.invoke('excluirUsuarioCompleto', { user_email });
      const vincs = await api.entities.VinculoEmpresa.filter({ userEmail: user_email, empresaId: empId });
      await Promise.all(vincs.map(v => api.entities.VinculoEmpresa.delete(v.id)));
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["userProfiles", empresaId] });
  ...
  const bloquearUsuarioMutation = useMutation({
    mutationFn: ({ id, bloqueado }) => api.entities.UserProfile.update(id, { bloqueado }),
    onSuccess: (_, vars) => {
      queryClient.invalidateQueries({ queryKey: ["userProfiles", empresaId] });
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <motion.button
              ...
              onClick={async () => {
                ...
                try {
                  const res = await api.functions.invoke('sincronizarNomesVinculos', { empresaId });
                  const data = res?.data;
