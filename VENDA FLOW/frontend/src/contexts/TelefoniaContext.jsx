...
import { SOCKET_EVENTS, MENSAGENS_FALHA_AGENTE } from './telefonia/TELEFONIA_ENGINE';
...
export function TelefoniaProvider({ children }) {
    ...
    try {
      ...
      if (info3c?.qualificacoes?.length) {
        ...
        if (empresaId) {
          ...
          api.functions.invoke('buscarQualificacoes3CPlus', {
            empresaId,
  ...
  const fecharModalAtendimento = useCallback(() => {
    ...
    if (temSessaoReal && finalizar) {
      finalizar('outro', {}, 'Encerrada sem registro pelo agente', undefined, null).catch(() => {
        ...
        api.functions.invoke('executarComando3CPlus', { empresaId, comando: 'acw-exit' }).catch(() => {});
      });
    } else if (empresaId) {
  ...
  // ── Avisos operacionais do 3C Plus (não mudam o estado da ligação) ──
  // O mesmo aviso pode chegar várias vezes seguidas (ex: list-empty a cada tentativa
  // de discagem); mostramos no máximo um por chave a cada 10 minutos.
  const avisosMostradosRef = useRef(new Map());
  const mostrarAvisoOperacional = useCallback((evento, data) => {
    const chave = `${evento}:${data?.list?.id ?? data?.agent?.id ?? ''}`;
    const agora = Date.now();
    if (agora - (avisosMostradosRef.current.get(chave) ?? 0) < 10 * 60 * 1000) return;
    avisosMostradosRef.current.set(chave, agora);

    switch (evento) {
      case SOCKET_EVENTS.AGENT_SCHEDULE:
        // Payload não documentado além de `agent` — logado para detalharmos o aviso depois
        console.info('[Telefonia] agent-schedule:', data);
        toast.info('Você tem um retorno agendado agora no 3C Plus.', { duration: 10000 });
        break;
      case SOCKET_EVENTS.LIST_EMPTY: {
        const nome = data?.list?.name ? ` "${data.list.name}"` : '';
        toast.warning(`A lista de contatos${nome} da campanha acabou. Avise o gestor para subir uma nova lista.`, { duration: 15000 });
        break;
      }
      case SOCKET_EVENTS.REACHED_MAX_ONLINE_AGENTS:
        // Durante o login quem avisa é o próprio fluxo de login (erro LIMITE_AGENTES)
        if (loginPendingRef?.current) break;
        toast.error('A empresa atingiu o limite de agentes logados no 3C Plus.', { duration: 15000 });
        break;
    }
  }, [loginPendingRef]);
  ...
  useEffect(() => {
    const token = cfgTelefonia?.agenteHabilitado;
    if (!token || !empresaId) return;
    restoreAgentState(token, true, cfgTelefonia?.dominio, empresaId);
  }, [cfgTelefonia?.agenteHabilitado, empresaId]);
  ...
  const { socketRef } = useTelefoniaSocket({
    ...
    onReconnect: () => restoreAgentState(cfgTelefonia?.agenteHabilitado, true, cfgTelefonia?.dominio, empresaId),
    onManualMode:   (data) => {
    ...
    onEventoAgente: (evento) => {
      if (MENSAGENS_FALHA_AGENTE[evento]) toast.error(MENSAGENS_FALHA_AGENTE[evento]);
      processarEventoAgente(evento);
    },
    onLoginFailed: (data) => {
  ...
  const ligarAgoraLead = useCallback(async (leadId, leadNome) => {
    ...
    try {
      const resultado = await api.functions.invoke('ligarAgoraLead3CPlus', {
        empresaId,
        lead_id: leadId,
      });
  ...
  const iniciarLigacaoManual = useCallback(async ({ telefone_manual, lead_nome, lead_id } = {}) => {
    ...
    try {
      const resp = await api.functions.invoke('iniciarDiscagemManual3CPlus', {
        empresaId,
  ...
  const finalizarLigacao = useCallback(async (resultado, spin, observacao, novoStatusLead, qualificationId) => {
    ...
    try {
      const resp = await api.functions.invoke('finalizarLigacao3CPlus', {
        empresaId,
        call_session_id: sessao.id,
        resultado,
        spin: spin || null,
  ...
  const iniciarLigacao = useCallback(async (leadId, leadNome, opcoes = {}) => {
    ...
    setLeadCallStatus('discando');
    try {
      const resp = await api.functions.invoke('iniciarLigacao3CPlus', {
  ...
  }, [cfgTelefonia?.agenteHabilitado, cfgTelefonia?.dominio, restoreAgentState, empresaId]);
