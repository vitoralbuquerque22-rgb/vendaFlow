...
// Cada teste recebe { empresaId, user, userProfile, integracao, base44 }
  ...
  return [
    // ── SEÇÃO 1: Configuração ──────────────────────────────────
    {
      ...
      run: async ({ empresaId }) => {
        ...
        const profiles = await base44.entities.UserProfile.filter({ empresaId });
    ...
    // ── SEÇÃO 2: API 3C Plus — leitura ───────────────────────
    {
      ...
      run: async ({ empresaId }) => {
        ...
        const res = await base44.functions.invoke("testar3CPlus", {
          empresaId,
    ...
    // ── SEÇÃO 3: Entidades CRM ───────────────────────────────
    {
      ...
      run: async ({ empresaId }) => {
        const leads = await base44.entities.Lead.filter({ empresaId });
    ...
    },
    {
      ...
      run: async ({ empresaId, user }) => {
        const sessao = await base44.entities.CallSession.create({
          empresaId,
          lead_id: "smoke-test",
        ...
        await base44.entities.CallSession.update(sessao.id, {
          status: "finished",
    ...
    // ── SEÇÃO 4: Functions backend ────────────────────────────
    {
      ...
      run: async ({ empresaId, user }) => {
        ...
        let resultado = null;
        try {
          resultado = await base44.functions.invoke("ligarAgoraLead3CPlus", {
            empresaId,
            lead_id: lead.id,
          });
        } catch (e) {
          // Limpar lead antes de lançar
          await base44.entities.Lead.update(lead.id, { is_locked_for_call: false }).catch(() => {});
    ...
    },
    {
      ...
      run: async ({ empresaId }) => {
        try {
          await base44.functions.invoke("ligarAgoraLead3CPlus", {
            empresaId,
    ...
    },
    {
      ...
      run: async ({ empresaId, user }) => {
        ...
        let res;
        try {
          res = await base44.functions.invoke("finalizarLigacao3CPlus", {
            empresaId,
