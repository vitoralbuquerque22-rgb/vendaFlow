...
export default function ConviteUsuarioModal({ open, onOpenChange, empresaId, onConviteSuccess }) {
  ...
  const handleConvidar = async () => {
    ...
    try {
      ...
      const vinculosExistentes = await base44.entities.VinculoEmpresa.filter({ empresaId, userEmail: email.trim() });
      ...
      const convitesPendentes = await base44.entities.ConviteEmpresa.filter({ empresaId, email: email.trim(), status: "pendente" });
