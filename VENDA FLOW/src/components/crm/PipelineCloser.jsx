...
function ModalAcao({ lead, acao, onClose, onConfirmar, cadencias, currentUser }) {
  ...
  const { data: colaboradores = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await base44.functions.invoke('buscarColaboradoresEmpresa', { empresaId: empresaIdModal });
  ...
  const { data: leadsRaw = [], isLoading: loadingLeads } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [comoCloser, comoSdr] = await Promise.all([
        base44.entities.Lead.filter({ empresaId, closer_responsavel: user.email }, "-created_date", 200),
        base44.entities.Lead.filter({ empresaId, sdr_responsavel: user.email }, "-created_date", 200),
      ]);
