...
export default function SeletorCampanha() {
  ...
  useEffect(() => {
    ...
    async function carregar() {
      ...
      try {
        ...
        // Buscar token do UserProfile — para verificação de configuração
        const profile = await buscarUserProfile(user.email);
        const tokenDoProfile = profile?.token_3cplus || null;
        const mapeamento = integracao.configuracao?.mapeamento_agentes || {};
        const emailNorm  = (user.email || '').toLowerCase();
        const tokenLegado = mapeamento[emailNorm] || mapeamento[user.email] || integracao.configuracao?.token_agente || null;
        const token = tokenDoProfile || tokenLegado || null;
        // CRÍTICO: token do UserProfile é criptografado (enc:) — decriptar
        // antes de usar na extensão/WebRTC, senão o 3C nunca autentica.
        ...
        let lista = [];
        try {
          const campanhasResp = await base44.functions.invoke('executarComando3CPlus', {
            empresaId,
          ...
          try {
            const rLogged = await base44.functions.invoke('executarComando3CPlus', {
              empresaId,
  ...
  const entrarCampanha = async (campanha) => {
    ...
    try {
      ...
      await loginCampanha(campanha.id, { token: tokenAgente, dominio: dominioApp });
  ...
  const salvarHabilitadas = async () => {
    ...
    try {
      const integracoes = await base44.entities.Integracao.filter({ id: integracaoId });
