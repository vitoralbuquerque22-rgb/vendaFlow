  ...
  const mutation = useMutation({
    mutationFn: async (data) => {
      ...
      if (data.full_name?.trim()) {
        try {
          const vinculos = await base44.entities.VinculoEmpresa.filter({ userEmail: user.email });
          await Promise.all(
            vinculos.map(v => base44.entities.VinculoEmpresa.update(v.id, { userName: data.full_name.trim() }))
          );
        } catch (e) {
