...
export default function Layout({ children, currentPageName }) {
  ...
  useEffect(() => {
    const atualizarAcesso = async () => {
      if (empresaId && user?.email) {
        try { await base44.functions.invoke('atualizarUltimoAcesso', { empresaId }); } catch {}
      }
    };
    atualizarAcesso();
  ...
  const { data: tarefasPendentes = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!empresaId) return [];
      return await base44.entities.Tarefa.filter({ empresaId, status: "pendente" });
    },
    enabled: !!empresaId,
  });
  ...
  const handleLogout = () => { base44.auth.logout(createPageUrl("Acesso")); };
