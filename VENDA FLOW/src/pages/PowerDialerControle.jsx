...
export default function PowerDialerControle() {
  ...
  const { data: vinculos = [] } = useQuery({
    ...
    queryFn: () => empresaId ? base44.entities.VinculoEmpresa.filter({ empresaId, status: "ativo" }) : [],
    enabled: !!empresaId
  });
  ...
  const handlePreview = async () => {
    ...
    setPreviewData(null);
    try {
      const res = await base44.functions.invoke("powerDialer3CPlus", {
        empresaId,
        modo: "preview",
        quantidade,
  ...
  const carregarDados3C = async (forceEmpresaId) => {
    ...
    try {
      const res = await base44.functions.invoke("buscarDadosCampanha3CPlus", { empresaId: eid });
  ...
  const carregarCampanhasCriadas = async () => {
    ...
    try {
      const res = await base44.entities.CampanhaVendaFlow.filter({ empresaId });
      ...
      const campanhasComStatus = await Promise.all((res || []).map(async (c) => {
        ...
        try {
          const det = await base44.functions.invoke("gerenciarCampanha3CPlus", {
          ...
          if (statusReal !== c.status) {
            base44.entities.CampanhaVendaFlow.update(c.id, { status: statusReal }).catch(() => {});
          }
  ...
  const buscarMetricasCampanha = async (campanha) => {
    ...
    try {
      const res = await base44.functions.invoke("buscarMetricasCampanha3CPlus", {
        empresaId,
  ...
  const buscarLeadsCRM = async () => {
    ...
    try {
      ...
      const leads = await base44.entities.Lead.filter(filtro);
  ...
  const buscarCampanhas3CPlus = async (forceEmpresaId) => {
    ...
    try {
      const res = await base44.functions.invoke("listarCampanhas3CPlus", { empresaId: eid });
  ...
  const buscarAgentes3C = async (campanhaId) => {
    try {
      const res = await base44.functions.invoke("gerenciarCampanha3CPlus", {
        empresaId,
  ...
  const executarAcao3C = async (campanha, acao, dados = null) => {
    ...
    try {
      ...
      const [res] = await Promise.all([
        base44.functions.invoke("gerenciarCampanha3CPlus", {
          empresaId,
          campanha_id_3cplus: campanha.id_3cplus,
          acao,
          ...(dados ? { dados } : {}),
  ...
  const handleCriarCampanha = async () => {
    ...
    try {
      const res = await base44.functions.invoke("criarCampanha3CPlus", {
        empresaId,
        nome: campanhaNome,
