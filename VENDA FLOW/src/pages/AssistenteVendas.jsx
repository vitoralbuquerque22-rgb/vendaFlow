...
export default function AssistenteVendas() {
  ...
  const { data: analisesIA = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!empresaId) return [];
      const allAnalises = await base44.entities.Atividade.filter({ empresaId, tipo: "anotacao" }, "-created_date", 1000);
  ...
  const { data: leads = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!empresaId) return [];
      const allLeads = await base44.entities.Lead.filter({ empresaId }, "-updated_date", 100);
  ...
  const { data: avaliacoesProdutos = [] } = useQuery({
    ...
    queryFn: () => base44.entities.AvaliacaoProduto.filter({ empresaId }, "-created_date"),
    enabled: isGestorOrAdmin && !!empresaId,
  });

  const { data: equipes = [] } = useQuery({
    queryKey: ["equipes", empresaId],
    queryFn: () => base44.entities.Equipe.filter({ empresaId }),
    enabled: isGestorOrAdmin && !!empresaId,
  });
  ...
  useEffect(() => {
    ...
    const unsubscribe = base44.agents.subscribeToConversation(conversacaoId, (data) => {
  ...
  const enviarMensagem = async () => {
    ...
    setMensagem("");
    try {
      const conversa = await base44.agents.getConversation(conversacaoId);
      await base44.agents.addMessage(conversa, { role: "user", content: textoMensagem });
    } catch (error) {
      console.error(error);
  ...
  const handleUpload = async (event) => {
    ...
    try {
      ...
      if (isAudio) {
        ...
        const { data } = await base44.functions.invoke('transcreverAudio', { file_url });
        if (data.sucesso && data.transcricao) {
          const conversa = await base44.agents.getConversation(conversacaoId);
          await base44.agents.addMessage(conversa, { role: "user", content: `Analise esta transcrição de ligação:\n\n${data.transcricao}` });
        ...
        const conversa = await base44.agents.getConversation(conversacaoId);
        await base44.agents.addMessage(conversa, { role: "user", content: `Analise este arquivo (transcrição de ligação ou material de treinamento):`, file_urls: [file_url] });
