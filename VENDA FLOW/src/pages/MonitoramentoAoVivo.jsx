...
export default function MonitoramentoAoVivo() {
  ...
  const { data: integracoes } = useQuery({
    ...
    queryFn: () => base44.entities.Integracao.filter({ empresaId, tipo: 'telefonia', ativa: true }),
    enabled: !!empresaId,
    staleTime: 5 * 60 * 1000,
  });
  const tokenGestor = integracoes?.[0]?.configuracao?.token_gestor || null;
  ...
  } = useMonitoramentoRealTime({ empresaId, tokenGestor, habilitado: !!tokenGestor });
  ...
  const carregarIntervalos = async () => {
    ...
    try {
      const res = await base44.functions.invoke('acaoAgente3CPlus', { empresaId, acao: 'listar_intervalos' });
  ...
  const buscarInfoCliente = async (agente) => {
    ...
    try {
      ...
      let lead = null;
      if (sessao.lead_id) {
        const leads = await base44.entities.Lead.filter({ id: sessao.lead_id });
        lead = leads?.[0] || null;
      }
  ...
  const executarAcao = async (agente, acao, params = {}) => {
    ...
    try {
      const res = await base44.functions.invoke('acaoAgente3CPlus', {
      ...
      if (ok) {
        ...
        // Log de auditoria
        base44.entities.MonitoramentoLog.create({
          empresaId,
