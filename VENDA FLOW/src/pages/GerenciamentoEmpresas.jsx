...
export default function GerenciamentoEmpresas() {
  ...
  const deleteEmpresaMutation = useMutation({
    mutationFn: (empresaId) => base44.functions.invoke('excluirEmpresaCompleta', { empresaId }),
    onSuccess: (response) => {
  ...
  const reenviarConviteMutation = useMutation({
    mutationFn: async (empresa) => {
      const response = await base44.functions.invoke('reenviarConviteEmpresa', {
  ...
  const validarBanco = async () => {
    setValidando(true);
    try {
      const response = await base44.functions.invoke('validarBancoDados', {});
  ...
  const limparOrfaos = async () => {
    setLimpando(true);
    try {
      const response = await base44.functions.invoke('limparRegistrosOrfaos', {});
