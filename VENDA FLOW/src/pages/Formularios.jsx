...
export default function Formularios() {
  ...
  const { data: formularios = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.Formulario.filter({ empresaId }),
    enabled: !!empresaId,
  });

  const { data: produtos = [] } = useQuery({
    queryKey: ["produtos", empresaId],
    queryFn: () => base44.entities.Produto.filter({ empresaId, ativo: true }),
    enabled: !!empresaId,
  });
  ...
  const { data: analisesIA = [] } = useQuery({
    ...
    queryFn: async () => {
      const atividades = await base44.entities.Atividade.filter({ empresaId, tipo: "anotacao" }, "-created_date", 100);
  ...
  const criarFormularioMutation = useMutation({
    mutationFn: (data) => { const p = { ...data }; if (!p.empresaId) p.empresaId = empresaId; return base44.entities.Formulario.create(p); },
  ...
  const atualizarFormularioMutation = useMutation({
    mutationFn: ({ id, data }) => base44.entities.Formulario.update(id, data),
  ...
  const deletarFormularioMutation = useMutation({
    mutationFn: (id) => base44.entities.Formulario.delete(id),
  ...
  const gerarFormularioComIA = async () => {
    ...
    try {
      ...
      const response = await base44.integrations.Core.InvokeLLM({ prompt, response_json_schema: { type: "object", properties: { nome: { type: "string" }, descricao: { type: "string" }, perguntas: { type: "array", items: { type: "object", properties: { pergunta: { type: "string" }, tipo_resposta: { type: "string" }, opcoes: { type: "array", items: { type: "string" } }, obrigatoria: { type: "boolean" } } } } } } });
