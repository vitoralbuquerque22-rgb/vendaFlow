    ...
    const autoSelectOrRedirect = async () => {
      try {
        ...
        if (!vinculos || vinculos.length === 0) {
          ...
          try {
            const convites = await base44.entities.ConviteEmpresa.filter({
              email: user.email,
              status: "pendente",
            });
            ...
            if (convites && convites.length > 0) {
              ...
              await base44.entities.ConviteEmpresa.update(convite.id, { status: 'aceito' });

              // Setar empresa atual
              await base44.auth.updateMe({ empresaAtualId: convite.empresaId });
