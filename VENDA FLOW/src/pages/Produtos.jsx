...
export default function Produtos() {
  ...
  const { data: produtos = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.Produto.filter({ empresaId }),
    enabled: !!empresaId,
  });

  const { data: categorias = [] } = useQuery({
    queryKey: ["categorias-produto"],
    queryFn: () => base44.entities.CategoriaProduto.filter({ ativa: true }),
  });
  ...
  const criarMutation = useMutation({
    mutationFn: (data) => base44.entities.Produto.create({ ...data, empresaId }),
  ...
  const gerarManual = async (produto) => {
    ...
    try {
      ...
      const perguntas = await base44.integrations.Core.InvokeLLM({
