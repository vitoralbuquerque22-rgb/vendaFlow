...
export default function BoasVindas() {
    ...
    const load = async () => {
      try {
        ...
        if (vinculos?.length > 0) {
          ...
          const empresas = await base44.entities.Empresa.filter({ id: vinculos[0].empresaId });
          if (empresas?.length > 0) setEmpresa(empresas[0]);
        }

        // Pré-carregar dados do perfil se já existir
        const perfis = await base44.entities.UserProfile.filter({ user_email: u.email });
        if (perfis?.length > 0) {
          const p = perfis[0];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const perfis = await base44.entities.UserProfile.filter({ user_email: user.email });
      const profileData = {
        user_name: nome.trim(),
      ...
      if (perfis?.length > 0) {
        await base44.entities.UserProfile.update(perfis[0].id, profileData);
      }
