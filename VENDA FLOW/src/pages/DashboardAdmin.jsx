...
export default function DashboardAdmin() {
  ...
  const { data: empresas = [], isLoading: loadingEmpresas } = useQuery({ queryKey: ["all-empresas"], queryFn: () => base44.entities.Empresa.list(), enabled: isSuperAdmin });
  const { data: vinculos = [], isLoading: loadingVinculos } = useQuery({ queryKey: ["all-vinculos"], queryFn: () => base44.entities.VinculoEmpresa.list(), enabled: isSuperAdmin });
  const { data: leads = [], isLoading: loadingLeads } = useQuery({ queryKey: ["all-leads"], queryFn: () => base44.entities.Lead.list("-created_date", 10000), enabled: isSuperAdmin });
  const { data: atividades = [], isLoading: loadingAtividades } = useQuery({ queryKey: ["all-atividades"], queryFn: () => base44.entities.Atividade.list("-created_date", 5000), enabled: isSuperAdmin });
  const { data: tarefas = [], isLoading: loadingTarefas } = useQuery({ queryKey: ["all-tarefas"], queryFn: () => base44.entities.Tarefa.list("-created_date", 5000), enabled: isSuperAdmin });
