...
export default function EmailMarketing() {
  ...
  const { data: templates = [] } = useQuery({
    ...
    queryFn: () => base44.entities.EmailTemplate.filter({ empresaId }),
    enabled: !!empresaId,
  });
  ...
  const { data: envios = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const lista = await base44.entities.EmailEnvio.filter({ empresaId }, "-created_date");
  ...
  const { data: leads = [] } = useQuery({
    ...
    queryFn: () => base44.entities.Lead.filter({ empresaId }),
    enabled: !!empresaId,
  });

  const { data: campanhasAgendadas = [] } = useQuery({
    queryKey: ["campanhas-agendadas", empresaId],
    queryFn: () => base44.entities.CampanhaAgendada.filter({ empresaId }, "-created_date"),
    enabled: !!empresaId,
  });
