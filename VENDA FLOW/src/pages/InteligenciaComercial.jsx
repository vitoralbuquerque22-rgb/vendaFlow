...
export default function InteligenciaComercial() {
  ...
  const { data: snapshot, isLoading, refetch, isFetching } = useQuery({
    ...
    queryFn: async () => {
      ...
      const rows = await base44.entities.CommercialInsights.filter({ empresaId, dia: hoje });
      return rows?.[0] || null;
    },
    enabled: !!empresaId,
    staleTime: 1800000,
  ...
  const handleRefresh = async () => {
    try { await base44.functions.invoke("calcularCommercialInsights", { empresaId }); } catch {}
    refetch();
  };
