...
export default async (req) => {
  try {
    ...
    const empresas = await api.asServiceRole.entities.Empresa.list();
    const idsEmpresas = new Set(empresas.map(e => e.id));

    // Buscar todos os vínculos
    const vinculos = await api.asServiceRole.entities.VinculoEmpresa.list();
    const vinculosOrfaos = vinculos.filter(v => !idsEmpresas.has(v.empresaId));

    // Buscar todos os convites
    const convites = await api.asServiceRole.entities.ConviteEmpresa.list();
