...
export default async (req) => {
  try {
    ...
    const logsAntigos = await api.asServiceRole.entities.LogAcesso.filter({
      registrado_em: { $lt: seteDiasAtras }
    }, "registrado_em", 100);
    if (logsAntigos.length > 0) {
      await Promise.all(logsAntigos.map(l => api.asServiceRole.entities.LogAcesso.delete(l.id)));
    }
