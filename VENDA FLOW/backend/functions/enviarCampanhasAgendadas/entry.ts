...
export default async (req) => {
  try {
    ...
    const campanhasPendentes = await api.asServiceRole.entities.CampanhaAgendada.filter({
      status: 'pendente'
    });
    ...
    for (const campanha of campanhasParaEnviar) {
      try {
        ...
        // Buscar template
        const template = await api.asServiceRole.entities.EmailTemplate.read(campanha.template_id);
        
        if (!template) {
          await api.asServiceRole.entities.CampanhaAgendada.update(campanha.id, {
            status: 'erro',
        ...
        const leads = await api.asServiceRole.entities.Lead.filter(leadsQuery);
        ...
        for (const lead of leadsElegiveis) {
          try {
            ...
            const envio = await api.asServiceRole.entities.EmailEnvio.create({
        ...
        if (campanha.sequencia_id) {
          const proximaCampanha = await api.asServiceRole.entities.CampanhaAgendada.filter({
          ...
          if (proximaCampanha.length > 0) {
            ...
            if (proxima.dias_apos_anterior) {
              ...
              await api.asServiceRole.entities.CampanhaAgendada.update(proxima.id, {
