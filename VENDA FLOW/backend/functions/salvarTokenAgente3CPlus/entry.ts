...
export default async (req) => {
  ...
  try {
    ...
    if (profiles[0]) {
      await api.asServiceRole.entities.UserProfile.update(profiles[0].id, updateData);
    } else {
      ...
      let role = 'sdr';
      const empresasOwner = await api.asServiceRole.entities.Empresa.filter({ ownerEmail: user.email });
      if (empresasOwner[0]) {
        role = 'admin';
      } else {
        const vinculos = await api.asServiceRole.entities.VinculoEmpresa.filter({ userEmail: user.email, status: 'ativo' });
