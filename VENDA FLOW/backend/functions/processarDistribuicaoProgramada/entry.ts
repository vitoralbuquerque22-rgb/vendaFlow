...
export default async (req) => {
  try {
    ...
    const programacoes = await api.asServiceRole.entities.ProgramacaoDistribuicao.filter({ ativa: true });
    ...
    for (const prog of programacoes) {
      ...
      if (isAfter(hojeDate, dataFim)) {
        await api.asServiceRole.entities.ProgramacaoDistribuicao.update(prog.id, { status: 'concluida', ativa: false });
        continue;
      }

      if (prog.status === 'aguardando') {
        await api.asServiceRole.entities.ProgramacaoDistribuicao.update(prog.id, { status: 'em_andamento' });
      }
      ...
      const todosLeads = await api.asServiceRole.entities.Lead.filter({ empresaId });
