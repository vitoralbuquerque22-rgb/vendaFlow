...
export default async (req) => {
  try {
    ...
    for (const empresa of empresas) {
      const automacoes = await api.asServiceRole.entities.AutomacaoRegra.filter({
        ativa: true,
      ...
      const [leads, tarefas, atividades] = await Promise.all([
        ...
        api.asServiceRole.entities.Atividade.filter({ empresaId: empresa.id }, "-created_date"),
      ]);
        ...
        try {
          ...
          for (const lead of leadsAfetados) {
            ...
            if (automacao.tipo === "criar_tarefa" && acoes.criar_tarefa) {
              ...
              const novaTarefa = await api.asServiceRole.entities.Tarefa.create({
                empresaId: empresa.id,
                lead_id: lead.id,
                lead_nome: lead.nome,
            ...
            if (automacao.tipo === "enviar_lembrete") {
              ...
              if (acoes.notificar_app) {
                await api.asServiceRole.entities.Alerta.create({
                  empresaId: empresa.id,
                  tipo: "anomalia",
            ...
            if (acaoExecutada) {
              await api.asServiceRole.entities.LogAutomacao.create({
