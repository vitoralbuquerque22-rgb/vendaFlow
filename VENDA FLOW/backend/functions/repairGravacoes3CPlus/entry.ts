...
export default async (req) => {
  ...
  try {
    ...
    const userProfile = await api.asServiceRole.entities.UserProfile
    ...
    const queryCalls = { start_date: startDate, end_date: endDate, per_page: 500, with_mailing: 'true' };

    console.log(`[repairGravacoes3CPlus] buscando calls dos últimos ${dias} dias...`);
    console.log(`[repairGravacoes3CPlus] URL: ${cred.baseUrl}/calls?${new URLSearchParams(queryCalls as any)} (token ${mascarar(cred.token)})`);

    const respCalls = await chamar3C(cred, '/calls', { query: queryCalls, timeoutMs: 30000 });
    if (!respCalls.ok) {
    ...
    const todas = await api.asServiceRole.entities.GravacaoLigacao.filter({ empresaId });
    ...
    for (const gravacao of precisamRepair) {
      try {
        ...
        if (Object.keys(update).length > 0) {
          await api.asServiceRole.entities.GravacaoLigacao.update(gravacao.id, update);
          corrigidos++;
        } else {
          jaCompletos++;
        }
