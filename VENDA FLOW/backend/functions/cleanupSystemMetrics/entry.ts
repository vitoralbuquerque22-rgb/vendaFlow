...
export default async (req) => {
  try {
    ...
    const all = await api.asServiceRole.entities.SystemMetric.list();
    ...
    for (const { ref, registros } of Object.values(grupos)) {
      ...
      const existing = await api.asServiceRole.entities.SystemMetric.filter({
      ...
      if (existing?.[0]) {
        await api.asServiceRole.entities.SystemMetric.update(existing[0].id, doc);
      } else {
        await api.asServiceRole.entities.SystemMetric.create(doc);
      }

      for (const r of registros) {
        await api.asServiceRole.entities.SystemMetric.delete(r.id);
        deletados++;
      }
      agregados++;
    }
    ...
    for (const m of velhos) {
      await api.asServiceRole.entities.SystemMetric.delete(m.id);
      deletados++;
    }
