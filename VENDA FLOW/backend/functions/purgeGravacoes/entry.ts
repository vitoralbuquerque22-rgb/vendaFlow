...
export default async (req) => {
  ...
  try {
    ...
    const gravacoes = await api.asServiceRole.entities.GravacaoLigacao.filter({
    ...
    for (const g of gravacoes) {
      ...
      if (new Date(g.expires_at) < agora) {
        try {
          await api.asServiceRole.entities.GravacaoLigacao.update(g.id, {
    ...
    const jobs = await api.asServiceRole.entities.TranscriptionJob.filter({
      empresaId: body.empresaId,
      status: 'processing',
    });
    let stuck_reset = 0;
    for (const job of jobs) {
      if (job.iniciado_em && job.iniciado_em < cincoMinutosAtras) {
        await api.asServiceRole.entities.TranscriptionJob.update(job.id, {
          status: 'stuck',
          stuck_detected_at: agora.toISOString(),
        }).catch(() => {});
        if (job.gravacao_id) {
          await api.asServiceRole.entities.GravacaoLigacao.update(job.gravacao_id, {
