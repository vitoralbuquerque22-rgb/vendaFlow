...
export default async (req) => {
  try {
    ...
    const envios = await api.asServiceRole.entities.EmailEnvio.filter({ id: envio_id });
    ...
    await api.asServiceRole.entities.EmailEnvio.update(envio_id, {
      clicou: true,
    ...
    if (lead_id) {
      try {
        await api.asServiceRole.functions.invoke('calcularLeadScore', { lead_id });
      } catch (err) {
