  ...
  try {
    ...
    const buscarSessaoAtiva = async () => {
      if (callSessionId) {
        ...
        const s = await api.asServiceRole.entities.CallSession.filter(filter);
        return s[0] || null;
      }
      if (email && empresaId) {
        // Sempre filtrar por empresaId para evitar cross-tenant
        const s = await api.asServiceRole.entities.CallSession.filter({ sdr_email: email, empresaId });
    ...
    switch (evento) {
      ...
      case 'agent-is-idle': {
        if (email && empresaId) {
          const p = await api.asServiceRole.entities.UserProfile.filter({ user_email: email, empresaId });
          if (p[0]) await api.asServiceRole.entities.UserProfile.update(p[0].id, {
      ...
      case 'agent-entered-manual': {
        ...
        if (s) await api.asServiceRole.entities.CallSession.update(s.id, { status: 'ringing' });
        break;
      }
      ...
      case 'call-was-connected': {
        ...
        if (s) {
          ...
          await api.asServiceRole.entities.CallSession.update(s.id, {
            status: 'answered',
            chamada_id_3cplus: chamadaIdNova,
            telephony_id_3cplus: telefonyId || s.telephony_id_3cplus,
            atendida_em: eventoTimestamp,
          });
          await api.asServiceRole.entities.Lead.update(s.lead_id, {
      ...
      case 'call-history-created': {
        ...
        if (s && s.empresaId) {
          ...
          const atividadesExistentes = await api.asServiceRole.entities.Atividade.filter({
            empresaId: s.empresaId,
