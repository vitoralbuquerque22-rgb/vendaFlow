...
export default async (req) => {
  try {
    ...
    // Buscar solicitação
    const requests = await api.asServiceRole.entities.PermissionChangeRequest.filter({
      id: request_id
    });
    ...
    await api.asServiceRole.entities.PermissionChangeRequest.update(request_id, {
      status: action,
    ...
    if (action === 'approved') {
      const userProfiles = await api.asServiceRole.entities.UserProfile.filter({
      ...
      if (userProfile) {
        ...
        if (request.request_type === 'page_access') {
          ...
          await api.asServiceRole.entities.UserProfile.update(userProfile.id, {
