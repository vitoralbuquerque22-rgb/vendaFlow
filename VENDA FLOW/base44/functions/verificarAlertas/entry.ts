...
Deno.serve(async (req) => {
  try {
    ...
    for (const empresa of empresas) {
      const configuracoes = await base44.asServiceRole.entities.ConfiguracaoAlerta.filter({
        ativo: true,
      ...
      const [leads, atividades, tarefas, configMeta] = await Promise.all([
        ...
        base44.asServiceRole.entities.Atividade.filter({ empresaId: empresa.id }),
        base44.asServiceRole.entities.Tarefa.filter({ empresaId: empresa.id }),
        base44.asServiceRole.entities.ConfiguracaoMeta.filter({ empresaId: empresa.id }),
      ]);
      ...
      for (const config of configuracoes) {
        ...
        if (deveMostrarAlerta) {
          ...
          const alertasRecentes = await base44.asServiceRole.entities.Alerta.filter({
          ...
          if (!jaTemAlertaRecente) {
            const novoAlerta = await base44.asServiceRole.entities.Alerta.create({
              empresaId: empresa.id,
              configuracao_alerta_id: config.id,
              tipo: config.tipo,
              titulo,
