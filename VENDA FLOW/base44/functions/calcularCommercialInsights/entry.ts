...
Deno.serve(async (req) => {
  try {
    ...
    const [leads, atividades, tarefas, stats] = await Promise.all([
      base44.asServiceRole.entities.Lead.filter({ empresaId }),
      base44.asServiceRole.entities.Atividade.filter({ empresaId }),
      base44.asServiceRole.entities.Tarefa.filter({ empresaId, status: "pendente" }),
      base44.asServiceRole.entities.SdrDailyStats.filter({ empresaId }),
    ]);
    ...
    const existing = await base44.asServiceRole.entities.CommercialInsights.filter({ empresaId, dia: hoje });
    const docData = {
      empresaId,
      dia: hoje,
    ...
    if (existing?.[0]) {
      await base44.asServiceRole.entities.CommercialInsights.update(existing[0].id, docData);
    } else {
      await base44.asServiceRole.entities.CommercialInsights.create(docData);
    }
