...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // URL base correta da API 3C Plus
    ...
    try {
      const respAgentes = await fetch(
        `${baseUrl}/agents?api_token=${encodeURIComponent(token)}&per_page=200`,
    ...
    // Montar manualmente para garantir encoding correto das datas (espaço = %20)
    let urlCalls = `${baseUrl}/calls?api_token=${encodeURIComponent(token)}`;
    urlCalls += `&start_date=${encodeURIComponent(startDate)}`;
    urlCalls += `&end_date=${encodeURIComponent(endDate)}`;
    urlCalls += `&per_page=200`;
    urlCalls += `&with_mailing=true`;
    urlCalls += `&minimum_duration=5`;
    ...
    if (filtroEmail) {
      ...
      if (agenteId) {
        urlCalls += `&agents%5B%5D=${agenteId}`; // agents[] encodado corretamente
    ...
    console.log('[buscarGravacoes3CPlus] GET', urlCalls.replace(token, 'TOKEN'));

    const respCalls = await fetch(urlCalls, { signal: AbortSignal.timeout(15000) });
    if (!respCalls.ok) {
    ...
    const gravacoesBanco = await base44.asServiceRole.entities.GravacaoLigacao.filter({ empresaId });
    ...
    for (const call of calls) {
      try {
        ...
        await base44.asServiceRole.entities.GravacaoLigacao.create({
          empresaId,
    ...
    if (registrosVazios.length > 0) {
      ...
      for (const reg of registrosVazios.slice(0, 100)) {
        ...
        try {
          await base44.asServiceRole.entities.GravacaoLigacao.update(reg.id, {
