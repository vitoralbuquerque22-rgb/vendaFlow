...
Deno.serve(async (req) => {
  try {
    ...
    if (payload.empresaId || user?.empresaAtualId) {
      ...
      const progsAtivas = await base44.asServiceRole.entities.ProgramacaoDistribuicao.filter({ ativa: true });
    ...
    for (const empresaId of empresaIds) {
      ...
      const atividades = await base44.asServiceRole.entities.Atividade.filter({ empresaId });
      const ultimaAtiv = {};
      ...
      const tarefasPendentes = await base44.asServiceRole.entities.Tarefa.filter({ empresaId, status: 'pendente' });
      ...
      const leadsAtivos = await base44.asServiceRole.entities.Lead.filter({ empresaId });
