...
Deno.serve(async (req) => {
  try {
    ...
    if (user.role !== 'admin') {
      const adminVinculo = await base44.asServiceRole.entities.VinculoEmpresa.filter({
    ...
    const vinculos = await base44.asServiceRole.entities.VinculoEmpresa.filter({ empresaId, userEmail });
    let vinculo;
    if (vinculos.length > 0) {
      vinculo = await base44.asServiceRole.entities.VinculoEmpresa.update(vinculos[0].id, { papel, status: 'ativo', userName });
    } else {
      vinculo = await base44.asServiceRole.entities.VinculoEmpresa.create({ empresaId, userEmail, userName: userName || userEmail, papel, status: 'ativo' });
    }
    ...
    if (profiles.length > 0) {
      perfil = await base44.asServiceRole.entities.UserProfile.update(profiles[0].id, { empresaAtualId: empresaId });
    }
