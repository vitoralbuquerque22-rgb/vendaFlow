...
Deno.serve(async (req) => {
  try {
    ...
    const programacoes = await base44.asServiceRole.entities.ProgramacaoDistribuicao.filter({ empresaId, ativa: true });
    const prog = programacoes[0];

    // Buscar regras específicas por campanha/origem
    const regras = await base44.asServiceRole.entities.RegraDistribuicao.filter({ ativa: true, empresaId: lead.empresaId });
    ...
    const vinculosEmpresa = await base44.asServiceRole.entities.VinculoEmpresa.filter({ empresaId, status: 'ativo' });
    const papelPorEmail = {};
    ...
    const perfisEmpresa = await base44.asServiceRole.entities.UserProfile.filter({ empresaId });
    ...
    if (regraAplicavel && regraAplicavel.sdrs_atribuidos?.length > 0) {
      ...
      if (escolha) {
        ...
        await base44.asServiceRole.entities.RegraDistribuicao.update(regraAplicavel.id, { ultimo_indice_fila: escolha.indice });
      }
      ...
      if (escolha) {
        ...
        await base44.asServiceRole.entities.ProgramacaoDistribuicao.update(prog.id, { ultimo_indice_fila: escolha.indice });
      }
    }
    ...
    if (cadenciaId) {
      const cadencias = await base44.asServiceRole.entities.Cadencia.filter({ id: cadenciaId });
