...
Deno.serve(async (req) => {
  try {
    ...
    const atividades = await base44.entities.Atividade.filter({ lead_id, empresaId });
    const envios = await base44.entities.EmailEnvio.filter({ lead_id, empresaId });
    ...
    const scoresAnteriores = await base44.entities.LeadScore.filter({ lead_id });
    ...
    if (scoreAnterior) {
      await base44.entities.LeadScore.update(scoreAnterior.id, scoreData);
    } else {
